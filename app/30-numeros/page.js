...
export default function Home() {
  ...
  const enviarPalpite = async () => {
    ...
    // O caminho da API continua o mesmo, pois é absoluto
  ...
  if (verResultado && dadosResultado) {
    return (
      <div className="min-h-screen p-8 bg-gray-900 text-white">
        ...
        {/* Botão atualizado para voltar para a HOME */}
        <button onClick={() => window.location.href = '/'} className="mt-8 bg-blue-600 px-4 py-2 rounded">Sair do Admin</button>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-green-900 text-white p-4 text-center">
        ...
        <div className="flex flex-col gap-4 mt-8">
          <button onClick={() => window.location.href = '/'} className="bg-white text-green-900 px-6 py-2 rounded-full font-bold">Voltar ao Início</button>
          <button onClick={() => window.location.reload()} className="text-sm underline opacity-80">Novo cadastro (Mesma pessoa)</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-4 max-w-2xl mx-auto font-sans">
      ...
      {/* Botão para voltar para a Home caso tenha clicado errado */}
      <button onClick={() => window.location.href = '/'} className="mb-4 text-sm text-gray-500 flex items-center">⬅ Voltar</button>
